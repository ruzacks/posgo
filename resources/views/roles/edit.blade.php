                                            ...
                                            @foreach ($internalPermission as $ip)
                                                @if (in_array($ip . ' ' . $module, $permissions))
                                                    ...
                                                    <div class="col-3 custom-control form-check">
                                                        {{ Form::checkbox('permissions[]', $key, $role->permissions, ['class' => 'form-check-input', 'id' => 'permission_' . $key]) }}
                                                        {{ Form::label('permission_' . $key, $ip, ['class' => 'form-check-label']) }}
                                                    </div>
                                                @endif
                                            @endforeach
                                        ...
                                        @foreach ($customPermission as $p)
                                            @if (in_array($p, $permissions))
                                                ...
                                                <div class="col-4 custom-control form-check">
                                                    {{ Form::checkbox('permissions[]', $key, $role->permissions, ['class' => 'form-check-input', 'id' => 'permission_' . $key]) }}
                                                    {{ Form::label('permission_' . $key, $p, ['class' => 'form-check-label']) }}
                                                </div>
                                            @endif
                                        @endforeach
